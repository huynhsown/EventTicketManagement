...
@Transactional
public class ChangeReservationQuantityService implements ChangeReservationQuantityUseCase {

    private final ReservationRepository reservationRepository;

    @Override
    public ReservationResult execute(ChangeReservationQuantityCommand cmd) {
        Reservation reservation = reservationRepository.findById(cmd.getId())
                .orElseThrow(() -> new NotFoundException("Reservation not found"));

        reservation.ensureOwnedBy(cmd.getUserId());
        reservation.changeQuantity(cmd.getQuantity());
